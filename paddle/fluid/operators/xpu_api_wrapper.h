...
static void GetFCInfo(const phi::DDim& x_dims,
  ...
  // VLOG(0) << "x_dims = " << x_dims.to_str() << " y_dims = " << y_dims.to_str()
  // << " x_trans = " << trans_x << " y_trans = " << trans_y
  // << " mat_dim_a.height_ = " << mat_dim_a.height_
  // << " mat_dim_a.batch_size_ = " << mat_dim_a.batch_size_
  // << " mat_dim_b.height_ = " << mat_dim_b.height_
  // << " mat_dim_b.batch_size_ = " << mat_dim_b.batch_size_;
...
static void xpu_fc_batch_wrapper(xpu::Context* xpu_ctx,
                                 ...
                                 const float* x_maxptr,
                                 const float* w_maxptr) {
  // VLOG(0) << "xpu_fc_batch_wrapper, bs: " << bs << ", trans_x:" << trans_x
  // << " , trans_w: " << trans_w << ", m: " << m << ", n: " << n << ", k: " << k
